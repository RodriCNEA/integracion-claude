...
class MaquinaPanel(tk.Frame):
    ...
    def _build_main_tab(self, tab):
        ...
        # --- Fila superior: gráfico + gauge ---
        top = tk.Frame(pane, bg=THEME["bg"])
        pane.add(top, weight=3)

        graph_cont = tk.Frame(top, bg=THEME["bg3"],
                               highlightbackground=THEME["border"],
                               highlightthickness=1)
        graph_cont.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 4))
        ...
        gauge_cont = tk.Frame(top, bg=THEME["bg3"],
                               highlightbackground=THEME["border"],
                               highlightthickness=1, width=240)
        gauge_cont.pack(side=tk.LEFT, fill=tk.Y, padx=(4, 0))
        gauge_cont.pack_propagate(False)
        ...
        # --- Fila inferior: sensores + tabla ensayo activo ---
        bottom = tk.Frame(pane, bg=THEME["bg"])
        pane.add(bottom, weight=2)
        bottom.columnconfigure(0, weight=3)
        bottom.columnconfigure(1, weight=2)

        self._sensors_frame = tk.Frame(bottom, bg=THEME["bg"])
        self._sensors_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 4), pady=4)
        self._build_main_sensors(self._sensors_frame)

        # Tabla de ensayo activo (columna derecha)
        ens_cont = tk.Frame(bottom, bg=THEME["bg3"],
                             highlightbackground=THEME["border"],
                             highlightthickness=1)
        ens_cont.grid(row=0, column=1, sticky="nsew", padx=(4, 0), pady=4)
    ...
    def _build_ensayo_tab(self, tab):
        ...
        ttk.Entry(add_row, textvariable=self._attr_key, width=18).pack(side=tk.LEFT)
    ...
    def _toggle_theme(self):
        ThemeManager.toggle()
        apply_theme_to_style()
        self.winfo_toplevel().configure(bg=THEME["bg"])
