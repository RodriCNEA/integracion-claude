...
class PlotWidget(tk.Frame):
    ...
    def _redraw(self):
        ...
        if self._data:
            ...
            # Solo aplicar el formatter de fechas cuando hay rango real (>=2 puntos distintos)
            if len(times) >= 2 and (times[-1] - times[0]).total_seconds() > 0:
...
# ===========================================================================
#  ALARM BANNER — barra de alarma prominente pero no invasiva
# ===========================================================================
class AlarmBanner(tk.Frame):
    """
    Barra de alarma que aparece sobre el notebook cuando hay una falla activa.
    Usa pack(before=notebook) para insertarse correctamente sobre el contenido.
    """

    def __init__(self, parent, **kwargs):
        super().__init__(parent, bg=THEME["red"], **kwargs)
        self._nb_ref = None   # Se asigna desde _build_ui

        # Franja izquierda de acento (extra visible)
        accent_bar = tk.Frame(self, bg="#ff0000", width=6)
        accent_bar.pack(side=tk.LEFT, fill=tk.Y)

        # Icono
        tk.Label(self, text="🚨", bg=THEME["red"],
                 font=("Segoe UI", 16), pady=8).pack(side=tk.LEFT, padx=(10, 4))

        # Texto del mensaje
        self._msg = tk.Label(self, text="", bg=THEME["red"], fg="#fff",
                              font=("Segoe UI", 11, "bold"), pady=8,
                              anchor="w", justify="left")
        self._msg.pack(side=tk.LEFT, padx=(0, 16), expand=True, fill=tk.X)

        # Botón de reconocimiento
        self._btn = tk.Button(
            self, text="✔  RECONOCER ALARMA",
            bg="#1a0000", fg="#fca5a5",
            font=("Segoe UI", 9, "bold"),
        ...
        self._btn.pack(side=tk.RIGHT, padx=12, pady=6)

        # Contador de alarmas activas
        self._count_lbl = tk.Label(self, text="", bg=THEME["red"],
                                    fg="#fca5a5", font=("Segoe UI", 8))
        self._count_lbl.pack(side=tk.RIGHT, padx=(0, 4))

        self._visible = False
        self._blink_state = True
        self._blink_id = None

    def show(self, message: str, count: int = 1, on_ack=None):
        self._msg.config(text=f"  {message}")
        self._count_lbl.config(
            text=f"{count} alarma{'s' if count > 1 else ''} activa{'s' if count > 1 else ''}")
        if on_ack:
            self._btn.config(command=on_ack)
        if not self._visible:
            # Insertar ANTES del notebook para que quede en la parte superior
            if self._nb_ref:
                self.pack(fill=tk.X, side=tk.TOP, before=self._nb_ref)
            else:
    ...
    def _start_blink(self):
        """Pulsa el fondo entre rojo intenso y rojo oscuro para llamar la atención."""
        self._stop_blink()
        self._do_blink()
    ...
    def _do_blink(self):
        if not self._visible:
            return
        color = "#c41c1c" if self._blink_state else "#991414"
        self.configure(bg=color)
        self._msg.configure(bg=color)
        self._count_lbl.configure(bg=color)
        self._blink_state = not self._blink_state
        self._blink_id = self.after(800, self._do_blink)
...
class MaquinaPanel(tk.Frame):
    ...
    def __init__(self, parent, core: "MaquinaCore", **kwargs):
        ...
        # Vigilante del archivo de usuarios (auto-refresh lista)
        self._watch_users_file()
    ...
    def _build_ui(self):
        ...
        # Dar al banner referencia al notebook para pack(before=)
        self._alarm_banner._nb_ref = self._nb
    ...
    def _build_recording_buttons(self, parent):
        ...
        self._btn_stop = ttk.Button(parent, text="⏹  Finalizar",
                                     ...
                                     state=tk.DISABLED)
        self._btn_stop.pack(side=tk.LEFT, padx=(2, 6))

        # Cronómetro de ensayo
        self._timer_var = tk.StringVar(value="00:00:00")
        self._timer_running = False
        self._timer_paused_secs = 0
        timer_frame = tk.Frame(parent, bg=THEME["bg3"],
                               highlightbackground=THEME["border"],
                               highlightthickness=1)
        timer_frame.pack(side=tk.LEFT, padx=(2, 10))
        tk.Label(timer_frame, text="⏱", bg=THEME["bg3"],
                 fg=THEME["text_muted"], font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=(6, 2))
    ...
    def _build_ensayo_tab(self, tab):
        ...
        ttk.Label(r0, text="Ensayo (Configuración):").pack(side=tk.LEFT)
        # Le sacamos el state="readonly" para que pueda escribir nombres nuevos
        self._ensayo_combo = ttk.Combobox(r0, textvariable=self._ensayo_var, width=26)
        self._ensayo_combo.pack(side=tk.LEFT, padx=6)
        self._ensayo_combo.bind("<<ComboboxSelected>>", self._on_ensayo_selected)
        self._ensayo_combo.bind("<Return>", lambda e: self._save_ensayo()) # Atajo Enter
        ttk.Button(r0, text="Cargar activo",
                   command=self._load_ensayo).pack(side=tk.LEFT, padx=4)
        ttk.Button(r0, text="Guardar config.",
        ...
        ttk.Label(r1, text="Nombre de próxima Prueba:").pack(side=tk.LEFT)
        ttk.Entry(r1, textvariable=self._prueba_var, width=26).pack(side=tk.LEFT, padx=6)
        ttk.Label(r1, text="(Se agregará la hora al iniciar la grabación)",
                  foreground=THEME["text_muted"]).pack(side=tk.LEFT)
        ...
        self._predefined_attrs = [
            ...
            "Temperatura ambiente", "Humedad relativa", "Observaciones", "Otro...",
        ]
        self._attr_combo = ttk.Combobox(add_row, textvariable=self._attr_key,
        ...
        tk.Label(add_row, text="=", bg=THEME["bg"], fg=THEME["text_muted"]).pack(side=tk.LEFT, padx=4)
        attr_v_entry = ttk.Entry(add_row, textvariable=self._attr_val, width=30)
        attr_v_entry.pack(side=tk.LEFT)
        attr_v_entry.bind("<Return>", lambda e: self._add_attr()) # Atajo Enter
        
        ttk.Button(add_row, text="Agregar", command=self._add_attr).pack(side=tk.LEFT, padx=6)
        ttk.Button(add_row, text="Limpiar", command=self._clear_attrs).pack(side=tk.LEFT)

        # El Treeview queda igual:
        self._attr_tree = ttk.Treeview(attr, columns=("a", "v"), show="headings", height=8)
    ...
    def _on_tab_change(self, event):
        try:
            ...
            elif tab_text == "Web Server":
                # Fuerza una actualización apenas entrás a la pestaña
    ...
    def _build_config_tab(self, tab):
        ...
        for col, lbl in [(0, "Alarma"), (1, "Umbral"), (2, "Unidad"), (3, "Debounce")]:
        ...
        # Alarmas booleanas: sin umbral numérico (cable fusible, flujo de agua)
        ...
        for i, (key, alarm) in enumerate(self._core.alarms.items(), start=1):
            ...
            is_bool_alarm = key in BOOLEAN_ALARMS

            ttk.Checkbutton(lf_al, text=alarm.name,
                             variable=en).grid(row=i, column=0,
                                               sticky="w", padx=8, pady=3)
            if is_bool_alarm:
                # Alarma booleana: no tiene umbral, mostrar texto informativo
    ...
    def _start_recording(self):
        ...
        prueba_base = self._prueba_var.get().strip() or "Prueba"
        ts = datetime.now().strftime('%H%M%S')
        prueba = f"{prueba_base}_{ts}"
    ...
    def _pause_recording(self):
        ...
        # Pausar cronómetro: guardar acumulado hasta ahora
        if hasattr(self, "_timer_start") and self._timer_running:
            self._timer_paused_secs += (datetime.now() - self._timer_start).total_seconds()
            self._timer_running = False
    ...
    def _resume_recording(self):
        ...
        self._led_conn.set_ok("Grabando...")
        # Reanudar cronómetro desde ahora
        self._timer_start = datetime.now()
        self._timer_running = True
        self._tick_timer()
    ...
    def _tick_timer(self):
        ...
        if not getattr(self, "_timer_running", False):
            return
        elapsed = getattr(self, "_timer_paused_secs", 0)
    ...
    def _on_alarm(self, key, name, value, threshold):
        active_count = sum(1 for a in self._core.alarms.values() if a.active)
        msg = f"{name}  —  Valor: {value}  |  Umbral: {threshold}"
        self._alarm_banner.show(msg, count=active_count, on_ack=self._ack_alarm)
    ...
    def _ack_alarm(self):
        ...
        if nombre:
            # Enviar email de "en revisión" ANTES de marcar como ack
            self._core.notify_alarm_acknowledged(nombre)
    ...
    def _save_ensayo(self):
        ...
        if not nombre:
            messagebox.showwarning("Nombre vacío",
                                   "Ingresá un nombre de ensayo.", parent=self)
            return
        ...
        messagebox.showinfo("Guardado",
                            f"Configuración del ensayo '{nombre}' guardada.", parent=self)
    ...
    # Web Server — usuarios
    # -------------------------------------------------------------------
    def _watch_users_file(self):
        """Verifica si users.json cambió comparando su contenido y actualiza automáticamente."""
        try:
            import json
            current_users = self._load_users()
            # Convertimos a texto para comparar si hubo algún cambio real
            current_state = json.dumps(current_users, sort_keys=True)
            
            if not hasattr(self, "_last_users_state"):
                self._last_users_state = current_state
            elif self._last_users_state != current_state:
                self._last_users_state = current_state
                self._update_users_list()
        except Exception:
            pass
        finally:
            # El bloque 'finally' garantiza que el loop siga vivo pase lo que pase
            self.after(2000, self._watch_users_file)
